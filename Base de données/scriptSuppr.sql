...
BEGIN
	SELECT COUNT(Client.id), MONTH(Client.dateCreation) FROM Client where Client.idTypeClient = 3 AND YEAR(Client.dateCreation) = year GROUP BY MONTH(Client.dateCreation); 
...
CREATE DEFINER=`stage2021`@`localhost` PROCEDURE `getEvolutionParticulierByYear`(IN `year` INT(10))
    NO SQL
BEGIN
	SELECT COUNT(Client.id), MONTH(Client.dateCreation) FROM Client where Client.idTypeClient = 1 AND YEAR(Client.dateCreation) = year GROUP BY MONTH(Client.dateCreation); 
...
BEGIN
	SELECT COUNT(Client.id), MONTH(Client.dateCreation) FROM Client where Client.idTypeClient = 2 AND YEAR(Client.dateCreation) = year GROUP BY MONTH(Client.dateCreation); 
