...
require_once("/home/stage2021/domains/stage.hofmann.fr/public_html/sitereact/server/class/formule.class.php");
...
/**
 * Permet de communiquer avec la bdd pour effectuer des actions sur les formules
 */
...
class FormuleManager
{
     ...
     * @return $req resultat de la requête (array)
     */
     ...
     * @return $formules array contenant les Instances de formule
     */
    ...
    /**
     * Récupere tout l'historique
     * @return $formules array contenant les Instances de formule (formule = historique)
     */
    ...
    /**
     * Permet de récupérer les formules sous un autre format (contient en plus le nom des services) 
     * pour l'espace client
     * @param $idClient l'id du client que l'on souhaite récupérer ses formules
     * @return $req array contenant ses formules
     */
    public function getFormulesEspaceClient($idClient)
    {
        $req = $this->tDAO->query("CALL getFormulesEspaceClient(?);", array($idClient));
        return $req;
    }

    /**
     * Permet de récupérer toutes les modifications demandées par les clients
     * @return $modifications array contenant les modifications (formules)
     */
    public function getAllModification()
    {
        $modifications = $this->tDAO->query("CALL getAllModification();", array());
        return $modifications;
    }

    /**
     * Permet d'ajouter dans la bdd une modification (formule modifiée)
     * @param $modification la formule modifiée à ajouter
     */
    public function addModification($modification)
    {
        $req = $this->tDAO->call("CALL addModification(?,?,?,?,?,?,?);", $modification->toArray());
    }

    /**
     * Permet de supprimer une modification
     * @param $idFormule id du formule à modifier
     */
    public function deleteModification($idFormule)
    {
        $req = $this->tDAO->call("CALL deleteModification(?);", array($idFormule));
    }
}
